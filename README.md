...
2. **`/etc/ufw/after.init.d/`**: Директория, содержащая исполняемые скрипты, которые выполняются основным скриптом `after.init`. Каждый скрипт в этой директории может отвечать за инициализацию отдельного списка блокировки или белого листа (`ipset`) и настройку соответствующих правил `iptables`. Скрипты именуются с числовым префиксом (например, `05-`, `10-`, `20-`) для контроля порядка выполнения.\
Примечание: скрипт создания `ipset` белого листа должен выполняться раньше остальных, таким образом если нужный IP-адрес случайным образом окажется в списке блокировок он будет проигнорирован, так будет исполнятся позднее (правила применяются сверху-вниз)
...
* `UFBL_IPSET_NAME<Index>`: Имя `ipset`
* `UFBL_SEED_FILE<Index>`: Путь к файлу начального заполнения
